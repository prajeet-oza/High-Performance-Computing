...
def velVerlet(local_pos, local_vel, local_ljf, rank, size):
	...
	# calculating time spent in communication, P2 approach
	startv = MPI.Wtime()
...
def simulation(local_pos, local_vel, local_ljf, N_steps, N_out, index, pdata):
	...
	for i in range(N_steps):
		try:
			# calculating the time taken in communication, P2 approach
			start = MPI.Wtime()
...
# P2 approaching collection of all atom data
def parallelise(local_pos, rank, size):
	pos = np.zeros(N_atoms, dtype = position)

	logSize = int(np.ceil(np.log2(size)))

	# sending the data to the parent node from the leaves, and finally to the root
	for i in range(logSize):
		...
		if rank % div == 0:
			src = rank + div // 2
			if src < size:
				temp_size = comm.recv(source = src, tag = rank + 100)
				temp_pos = np.zeros(temp_size, dtype = position)
				temp_pos = comm.recv(source = src, tag = rank)
				local_pos = np.append(local_pos, temp_pos)
		elif rank % div == div // 2:
			dst = rank - div // 2
			comm.send(local_pos.shape[0], dest = dst, tag = 100 + dst)
			comm.send(local_pos, dest = dst, tag = dst)
		comm.Barrier() # synchronise

	if rank == 0:
		pos = local_pos.copy()

	# broadcasting all atom data from the root node
